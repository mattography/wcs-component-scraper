    ...
    <div>
      ...
      {isLoading ? (<div className="loading"><p>Loading components...</p><div className="loader"></div></div>):(
          ...
          <div className="components-details">
            <p>Data last updated: 7/18/2022</p> <Tooltip title="Download OCI components CSV"><Link href="#" underline="none" id="download-link" onClick={() => saveToCSV(rows)} download="wcs-oci-components.csv"><span>Download CSV</span><Download/></Link></Tooltip>
          </div>
          <p>There {rows.length >= 0 ? 'are' : 'is'} {rows.length > 1 && 'a combined '}<strong>{rows.length}</strong> {rows.length >= 0 ? 'components' : 'component'} {rows.length > 1 ? 'and their variants across all OCI pages' : ''} {props.a11yType != '' ? `that are ${props.a11yType}` : null} {props.searchFilter && `matching the term "${props.searchFilter}"`}</p>
