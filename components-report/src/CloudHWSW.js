...
function CloudHWSW(props) {
  const [data, setData] = useState();
  const [isLoading, setIsLoading] = useState(false);

  const getData=()=>{
    setIsLoading(true);
    fetch('./cleanedCloudHWSWIds.json')
  ...
  function Row(props) {
    ...
    function ComponentType(type) {
      let status = {
        'a11y-ready': <WarningAmber className="a11yUnknownTag"/>,
        'a11y-unknown': <QuestionMarkIcon className="a11yReadyTag"/>,
        ...
        <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
          ...
          <TableCell component="th" scope="row">
              <div className="a11yStatusWrapper">
                <span>{row.id}</span><span>{ComponentType(row.keyName)}</span>
              </div>
          </TableCell>
          <TableCell component="th" scope="row">
            {dedupedLinks.length}
    ...
    <div>
      ...
      {isLoading ? (<div className="loading"><p>Loading components...</p><div className="loader"></div></div>):(
          ...
          <div className="components-details">
            <p>Data last updated: 7/18/2022</p> <Tooltip title="Download Cloud HWSW components CSV"><Link href="#" underline="none" id="download-link" onClick={() => saveToCSV(rows)} download="wcs-cloudhwsw-components.csv"><span>Download CSV</span><Download/></Link></Tooltip>
          </div>
          <p>There {rows.length >= 0 ? 'are' : 'is'} {rows.length > 1 && 'a combined '}<strong>{rows.length}</strong> {rows.length >= 0 ? 'components' : 'component'} {rows.length > 1 ? 'and their variants across all Cloud HW and SW pages ' : ''} {props.a11yType != '' ? `that are ${props.a11yType}` : null} {props.searchFilter && `matching the term "${props.searchFilter}"`}</p>
          <TableContainer component={Paper}>
            <Table aria-label="collapsible table">
              ...
              <TableBody>
                  {rows.sort((a, b) => b.urlsCount - a.urlsCount).map(row => (
                    <Row keyName={row.name} row={row}/>
                  ))}
              </TableBody>
            </Table>
          </TableContainer>
